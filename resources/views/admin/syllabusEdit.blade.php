      ...
      <div class="container-fluid">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form method="post" action="{{asset('syllabus/edit')}}/{{ $data->id }}" enctype="multipart/form-data">
              @csrf
                <div class="card-body">
                    ...
                    <?php $Chapters = App\Models\Chapter::get(); ?>
                      <select class="form-control" name="chapter[]" multiple>
                        @foreach($Chapters as $chapter)
                          <option value="{{ $chapter->id }}" <?php if(in_array($chapter->id , explode(',',$data->chapter_ids))) { echo "selected"; } ?>>{{ $chapter->name }}</option>
                        @endforeach
                      </select>
                      @error('batch')
                        <div class="input-group-append">
                    ...
                    <?php $tests = App\Models\Test::get(); ?>
                      <select class="form-control" name="test[]" multiple>
                        @foreach($tests as $test)
                          <option value="{{ $test->id }}" <?php if(in_array($test->id , explode(',',$data->test_ids))) { echo "selected"; } ?>>{{ $test->name }}</option>
                        @endforeach
                      </select>
                      @error('test')
                        <div class="input-group-append">
